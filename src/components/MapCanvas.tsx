...
export default function MapCanvas({ data, theme = 'light', onPersonSelect, onIntroEnd }: { data: MemorialPerson[], theme?: 'light' | 'dark', onPersonSelect: (p: MemorialPerson) => void, onIntroEnd: () => void }) {
  ...
  // Limit map points to speed up the app. On mobile, show fewer points to prevent lag.
  const displayData = useMemo(() => {
    return data.slice(0, isMobile ? 800 : 2000);
  }, [data, isMobile]);
  ...
  return (
    <div className={`absolute inset-0 z-0 ${isDark ? 'bg-[#0B0F19]' : 'bg-[#e5e7eb]'}`}>
      ...
      >
        ...
        <Polyline positions={palestineBorder} color="#C41E3A" weight={4} opacity={0.9} dashArray="10,8" lineCap="round" />
